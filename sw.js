<!--
PASIECZNIK ULTRA
© 2025 Justyna Poluszejko-Kłoda

Kod źródłowy objęty prawami autorskimi.
Repozytorium publiczne wyłącznie w celach demonstracyjnych.
Zasady użycia określa plik LICENSE.
-->
<!DOCTYPE html>
<html lang="pl">
<head>
  <meta charset="UTF-8"/>
  <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no"/>
  <title>Pasiecznik ULTRA PRO</title>
  <link rel="manifest" href="manifest.webmanifest">
  <meta name="theme-color" content="#ca8a04">
  <style>
    :root{
      --primary:#ca8a04; --primary-dark:#854d0e;
      --bg:#f1f5f9; --white:#fff; --text:#0f172a;
      --danger:#dc2626; --success:#16a34a; --blue:#2563eb;
      --slate:#334155; --purple:#7e22ce; --emerald:#059669; --orange:#f97316;
      --border:#e2e8f0;
    }
    body{font-family:system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif;background:var(--bg);color:var(--text);margin:0;padding-bottom:90px}
    .header{background:var(--primary);color:#fff;padding:12px;position:sticky;top:0;z-index:50;box-shadow:0 4px 6px -1px rgba(0,0,0,.1)}
    .header-top{display:flex;justify-content:space-between;align-items:center;gap:10px;margin-bottom:10px}
    .header-title{font-weight:1000;text-transform:uppercase;letter-spacing:1px;font-size:1.05rem}
    .nav-scroll{display:flex;gap:8px;overflow-x:auto;padding-bottom:5px;scrollbar-width:none}
    .nav-scroll::-webkit-scrollbar{display:none}
    .container{max-width:1200px;margin:0 auto;padding:16px}
    .grid{display:grid;gap:16px}
    .cols-2{grid-template-columns:repeat(2,1fr)}
    @media (min-width:640px){.grid-hives{grid-template-columns:repeat(3,1fr)}}
    @media (min-width:768px){.grid-hives{grid-template-columns:repeat(4,1fr)}}
    @media (min-width:1024px){.grid-hives{grid-template-columns:repeat(6,1fr)}}
    @media (max-width:639px){.grid-hives{grid-template-columns:repeat(2,1fr)}}
    .btn{border:none;padding:8px 16px;border-radius:10px;font-weight:1000;cursor:pointer;text-transform:uppercase;font-size:.75rem;transition:.2s;color:#fff;display:inline-flex;align-items:center;justify-content:center;gap:6px;white-space:nowrap}
    .btn:active{transform:scale(.96)}
    .btn-nav{background:var(--primary-dark);border:1px solid rgba(255,255,255,.18)}
    .btn-green{background:var(--success)}
    .btn-red{background:var(--danger)}
    .btn-blue{background:var(--blue)}
    .btn-slate{background:var(--slate)}
    .btn-orange{background:var(--orange)}
    .btn-ghost{background:transparent;color:var(--text);border:1px solid var(--border)}
    .btn-ghost:active{transform:scale(.98)}
    .btn-chip{background:#0f172a;color:#fff;border:1px solid rgba(255,255,255,.12);padding:8px 10px}
    .btn-chip.active{background:var(--blue)}
    .btn-chip.active-red{background:var(--danger)}
    .card{background:var(--white);border-radius:14px;padding:16px;box-shadow:0 1px 3px rgba(0,0,0,.1);border:1px solid var(--border)}
    .input{width:100%;padding:10px;border:1px solid #cbd5e1;border-radius:10px;font-size:14px;box-sizing:border-box;background:#fff}
    .input:focus{outline:2px solid var(--primary);border-color:transparent}
    .hidden{display:none!important}
    .flex{display:flex}
    .gap-1{gap:6px}
    .gap-2{gap:8px}
    .gap-3{gap:12px}
    .justify-between{justify-content:space-between}
    .items-center{align-items:center}
    .w-full{width:100%}
    .text-center{text-align:center}
    .mb-2{margin-bottom:8px}
    .mt-2{margin-top:8px}
    .mt-3{margin-top:12px}
    .hive-card{border-bottom:6px solid var(--primary);cursor:pointer;transition:transform .2s}
    .hive-card:hover{transform:translateY(-2px)}
    .text-alert{color:var(--danger);font-weight:1000;animation:blink 2s infinite}
    @keyframes blink{0%{opacity:1}50%{opacity:.55}100%{opacity:1}}
    details{border:1px solid var(--border);border-radius:12px;margin-bottom:10px;overflow:hidden;background:#fff}
    details>summary{background:#f8fafc;padding:12px;font-weight:1000;cursor:pointer;list-style:none;font-size:.8rem;text-transform:uppercase;color:#475569;display:flex;justify-content:space-between;align-items:center}
    details[open]>summary{background:#e2e8f0}
    .details-content{padding:12px;border-top:1px solid var(--border)}
    .pill{display:inline-flex;align-items:center;gap:6px;padding:6px 10px;border-radius:999px;font-weight:1000;font-size:11px;border:1px solid var(--border);background:#fff}
    .pill.bad{border-color:#fecaca;background:#fff1f2;color:#991b1b}
    .muted{color:#64748b;font-weight:1000;font-size:11px;text-transform:uppercase;letter-spacing:1px}
    .small{font-size:12px;color:#64748b;font-weight:700}
    .hr{height:1px;background:var(--border);margin:10px 0}
    .row{display:grid;grid-template-columns:1fr;gap:10px}
    @media(min-width:600px){.row.cols2{grid-template-columns:1fr 1fr}}
    .tinybtn{padding:6px 10px;font-size:.7rem;border-radius:10px}
    .kv{display:grid;grid-template-columns:1fr auto;gap:10px;align-items:center}
    .badge{display:inline-flex;align-items:center;gap:6px;font-weight:1000;font-size:11px;padding:6px 10px;border-radius:999px;border:1px solid var(--border);background:#fff}
    .badge.red{border-color:#fecaca;background:#fff1f2;color:#991b1b}
    .badge.green{border-color:#bbf7d0;background:#f0fdf4;color:#166534}
    .badge.blue{border-color:#bfdbfe;background:#eff6ff;color:#1d4ed8}

    /* Modal */
    .modal-overlay{position:fixed;inset:0;background:rgba(15,23,42,.55);display:none;align-items:flex-end;justify-content:center;padding:16px;z-index:9999}
    .modal-overlay.show{display:flex}
    .modal{width:min(820px,100%);background:#fff;border-radius:18px;border:1px solid var(--border);box-shadow:0 12px 40px rgba(0,0,0,.25);overflow:hidden;animation:slideUp .18s ease-out}
    @keyframes slideUp{from{transform:translateY(18px);opacity:.5}to{transform:translateY(0);opacity:1}}
    .modal-header{padding:14px 16px;border-bottom:1px solid var(--border);display:flex;justify-content:space-between;align-items:center;gap:10px}
    .modal-title{font-weight:1000;text-transform:uppercase;letter-spacing:1px;font-size:14px}
    .modal-body{padding:16px;display:grid;gap:10px}
    .modal-footer{padding:12px 16px;border-top:1px solid var(--border);display:flex;justify-content:flex-end;gap:8px}

    .help{font-size:12px;color:#475569;font-weight:700}
  </style>
</head>
<body>

<header class="header">
  <div class="header-top">
    <div class="flex items-center gap-2">
      <button id="backBtn" class="btn btn-nav hidden">⬅</button>
      <div id="appTitle" class="header-title">PASIECZNIK ULTRA</div>
    </div>
    <div class="flex gap-2">
      <button id="exportBtn" class="btn btn-green">⬇ KOPIA</button>
      <label class="btn btn-slate" style="cursor:pointer">
        ⬆ IMPORT
        <input id="importFile" type="file" accept="application/json" hidden>
      </label>
    </div>
  </div>

  <div class="nav-scroll" id="topNav">
    <button data-view="hives" class="btn btn-nav">🐝 Pasieka</button>
    <button data-view="calendar" class="btn btn-nav" style="background:#4f46e5">📅 Kalendarz</button>
    <button data-view="finance" class="btn btn-nav" style="background:var(--emerald)">💰 Finanse</button>
    <button data-view="inventory" class="btn btn-nav" style="background:var(--slate)">📦 Magazyn</button>
    <button data-view="knowledge" class="btn btn-nav" style="background:var(--purple)">🧠 Wiedza</button>
    <button id="addHiveBtn" class="btn btn-green">+ UL</button>
  </div>
</header>

<main class="container">
  <!-- PASIEKA -->
  <section id="viewHives" class="grid">
    <div class="card" style="border-left:5px solid var(--blue)">
      <div class="muted mb-2">FILTRY EKRANU GŁÓWNEGO</div>
      <div class="flex gap-2" style="flex-wrap:wrap">
        <button id="fltReviewBtn" class="btn btn-chip">🔎 DO PRZEGLĄDU</button>
        <button id="fltQueenBtn" class="btn btn-chip">👑 MATKA DO WYMIANY</button>
        <button id="fltClearBtn" class="btn btn-ghost" style="padding:8px 10px">WYCZYŚĆ</button>
      </div>
      <div class="small mt-2">
        „Do przeglądu” = ul z <b>terminem następnego przeglądu</b> na dziś / wstecz, albo (jeśli brak terminu) ostatni przegląd starszy niż <b>6 dni</b>.
        „Matka do wymiany” = <b>aktualna matka</b> starsza niż <b>24 miesiące</b>.
      </div>
    </div>
    <div id="hivesGrid" class="grid grid-hives"></div>
  </section>

  <!-- SZCZEGÓŁY ULA -->
  <section id="viewDetails" class="hidden grid">
    <div class="card" style="border-left:6px solid var(--primary)">
      <div class="muted">UL</div>
      <div id="detailsTitle" style="font-weight:1000;font-size:18px;margin-top:6px"></div>
      <div class="hr"></div>

      <details open>
        <summary>Metadane ula <span class="small">numer / typ / rok / termin</span></summary>
        <div class="details-content">
          <div class="row cols2">
            <input id="hLabel" class="input" type="text" placeholder="Numer / nazwa ula (np. 12, A1)">
            <input id="hPlace" class="input" type="text" placeholder="Położenie (np. Rząd 2 / 3 od lewej)">
          </div>
          <div class="row cols2 mt-2">
            <select id="hType" class="input">
              <option value="rodzina">Rodzina</option>
              <option value="odklad">Odkład</option>
            </select>
            <input id="hYear" class="input" type="number" placeholder="Rok (np. 2025)">
          </div>
          <div class="row cols2 mt-2">
            <input id="hNextVisit" class="input" type="date" title="Termin następnego przeglądu (ręcznie)">
            <button id="saveHiveMetaBtn" class="btn btn-blue">ZAPISZ</button>
          </div>
          <div class="help mt-2">Termin następnego przeglądu wpisujesz <b>konkretną datą</b> (ręcznie).</div>
        </div>
      </details>

      <details open>
        <summary>Przeglądy <span class="small">dodaj • edytuj • usuń</span></summary>
        <div class="details-content">
          <div class="row cols2">
            <input id="revDate" type="date" class="input">
            <input id="revNext" type="date" class="input" placeholder="Termin następnego przeglądu">
          </div>
          <textarea id="revNote" class="input" style="height:90px" placeholder="Notatka z przeglądu..."></textarea>
          <div class="row cols2">
            <button id="addReviewBtn" class="btn btn-green">DODAJ PRZEGLĄD</button>
            <button id="revClearBtn" class="btn btn-ghost" style="padding:10px 14px">WYCZYŚĆ POLA</button>
          </div>
          <div class="small mt-2">Po dodaniu przeglądu możesz edytować datę i treść.</div>
          <div class="hr"></div>
          <div id="revList" class="grid" style="gap:10px"></div>
        </div>
      </details>

      <details open>
        <summary>Karmienie <span class="small">historia karmień</span></summary>
        <div class="details-content">
          <div class="row cols2">
            <input id="feedDate" type="date" class="input">
            <input id="feedType" type="text" class="input" placeholder="Rodzaj pokarmu (np. syrop 3:2, ciasto)">
          </div>
          <div class="row cols2 mt-2">
            <input id="feedQty" type="number" step="0.01" class="input" placeholder="Ilość">
            <select id="feedUnit" class="input">
              <option value="kg">kg</option>
              <option value="l">l</option>
            </select>
          </div>
          <input id="feedNote" type="text" class="input mt-2" placeholder="Wolna notatka (opcjonalnie)">
          <div class="row cols2 mt-2">
            <button id="addFeedBtn" class="btn btn-orange">DODAJ KARMienie</button>
            <button id="feedClearBtn" class="btn btn-ghost" style="padding:10px 14px">WYCZYŚĆ POLA</button>
          </div>
          <div class="hr"></div>
          <div id="feedList" class="grid" style="gap:10px"></div>
        </div>
      </details>

      <details open>
        <summary>Matki <span class="small">historia + aktualna</span></summary>
        <div class="details-content">
          <div class="row cols2">
            <input id="qName" type="text" class="input" placeholder="Nazwa matki (np. AGA-12)">
            <input id="qBreederLine" type="text" class="input" placeholder="Linia / hodowca (opcjonalnie)">
          </div>

          <div class="row cols2 mt-2">
            <select id="qYear" class="input"></select>
            <select id="qMonth" class="input"></select>
          </div>

          <div class="row cols2 mt-2">
            <select id="qInsem" class="input">
              <option value="">Typ unasienniania (opcjonalnie)</option>
              <option value="naturalne">Naturalne</option>
              <option value="sztuczne">Sztuczne (AI)</option>
              <option value="nieznane">Nieznane</option>
            </select>
            <select id="qOpalit" class="input">
              <option value="">Kolor opalitki (opcjonalnie)</option>
              <option value="brak">Brak opalitki</option>
              <option value="biała">Biała</option>
              <option value="żółta">Żółta</option>
              <option value="czerwona">Czerwona</option>
              <option value="zielona">Zielona</option>
              <option value="niebieska">Niebieska</option>
            </select>
          </div>

          <div class="row cols2 mt-2">
            <select id="qGentle" class="input">
              <option value="">Łagodność (opcjonalnie)</option>
              <option value="1">1 (słaba)</option>
              <option value="2">2</option>
              <option value="3">3</option>
              <option value="4">4</option>
              <option value="5">5 (super)</option>
            </select>
            <label class="input" style="display:flex;align-items:center;gap:10px;cursor:pointer">
              <input id="qIsCurrent" type="checkbox" style="width:18px;height:18px">
              Ustaw jako <b>aktualną</b>
            </label>
          </div>

          <textarea id="qNote" class="input mt-2" style="height:90px" placeholder="Wolna notatka (opcjonalnie)"></textarea>
          <div class="row cols2 mt-2">
            <button id="addQueenBtn" class="btn" style="background:var(--purple)">DODAJ MATKĘ</button>
            <button id="qClearBtn" class="btn btn-ghost" style="padding:10px 14px">WYCZYŚĆ POLA</button>
          </div>

          <div class="hr"></div>
          <div id="queenStatus" class="small"></div>
          <div id="queenList" class="grid" style="gap:10px;margin-top:10px"></div>
        </div>
      </details>
    </div>
  </section>

  <!-- KALENDARZ -->
  <section id="viewCalendar" class="hidden">
    <div class="flex gap-2 items-center mb-2">
      <div class="pill">📅 KALENDARZ</div>
      <select id="calYear" class="input" style="max-width:200px"></select>
    </div>
    <div id="calendarGrid" class="grid cols-2" style="gap:10px"></div>
  </section>

  <!-- FINANSE -->
  <section id="viewFinance" class="hidden">
    <div class="card text-center" style="border-top:5px solid var(--emerald);margin-bottom:16px">
      <div class="muted">BILANS</div>
      <div id="finBalance" style="font-weight:1000;margin:6px 0;font-size:30px">0.00 PLN</div>
      <div class="flex justify-between" style="font-size:10px;font-weight:1000">
        <span style="color:var(--success)">PRZYCHÓD: <span id="finInc">0</span></span>
        <span style="color:var(--danger)">WYDATEK: <span id="finExp">0</span></span>
      </div>
    </div>

    <div class="card mb-2">
      <div class="row cols2">
        <select id="finType" class="input">
          <option value="expense">📉 WYDATEK</option>
          <option value="income">📈 PRZYCHÓD</option>
        </select>
        <input id="finDate" type="date" class="input">
      </div>
      <div class="row cols2 mt-2">
        <input id="finAmount" type="number" placeholder="PLN" class="input">
        <input id="finDesc" type="text" placeholder="Opis (np. Cukier)" class="input">
      </div>
      <div class="mt-2">
        <button id="addFinBtn" class="btn btn-green w-full">DODAJ</button>
        <div class="small mt-2">Data w finansach jest <b>Twoja (ręczna)</b>. Pozycje możesz usuwać.</div>
      </div>
    </div>

    <div id="finList" class="grid" style="gap:10px"></div>
  </section>

  <!-- MAGAZYN -->
  <section id="viewInventory" class="hidden">
    <div class="card mb-2" style="border-top:5px solid var(--slate)">
      <h3 style="margin:0 0 10px 0;font-weight:1000">Magazyn (historia stanów)</h3>

      <div class="row cols2">
        <select id="invItemSel" class="input"></select>
        <input id="invNewName" type="text" placeholder="lub wpisz nowy produkt (np. Cukier)" class="input">
      </div>

      <div class="row cols2 mt-2">
        <input id="invMoveDate" type="date" class="input">
        <input id="invMoveDelta" type="number" step="0.01" placeholder="+ / − ilość (np. 25 lub -10)" class="input">
      </div>

      <div class="row cols2 mt-2">
        <input id="invMoveNote" type="text" placeholder="Powód (np. zakup, karmienie, produkcja)" class="input">
        <button id="invAddMoveBtn" class="btn btn-slate">DODAJ RUCH</button>
      </div>
      <div class="small mt-2">
        Każdy wpis to <b>ruch magazynowy</b> z datą. Dzięki temu możesz sprawdzić stan na dowolny dzień.
      </div>
    </div>

    <div class="card mb-2" style="border-left:5px solid var(--blue)">
      <div class="muted mb-2">SPRAWDŹ STAN NA DZIEŃ</div>
      <div class="row cols2">
        <select id="invQueryItem" class="input"></select>
        <input id="invQueryDate" type="date" class="input">
      </div>
      <button id="invQueryBtn" class="btn btn-blue w-full mt-2">POKAŻ</button>
      <div id="invQueryResult" class="mt-2"></div>
    </div>

    <div class="card">
      <div class="muted mb-2">AKTUALNE STANY</div>
      <div id="invList" class="grid" style="gap:10px"></div>
    </div>
  </section>

  <!-- WIEDZA -->
  <section id="viewKnowledge" class="hidden">
    <div class="card mb-2" style="border-top:5px solid var(--purple)">
      <h3 style="margin:0 0 10px 0;font-weight:1000">Wiedza (tematy)</h3>
      <div class="row cols2">
        <input id="knowNewTopic" type="text" placeholder="Nowy temat (np. Odkłady)" class="input">
        <button id="addTopicBtn" class="btn" style="background:var(--purple)">DODAJ TEMAT</button>
      </div>
      <div class="row cols2 mt-2">
        <select id="knowTopic" class="input"></select>
        <input id="knowTitle" type="text" placeholder="Tytuł notatki" class="input">
      </div>
      <textarea id="knowBody" placeholder="Treść..." class="input" style="height:90px"></textarea>
      <button id="addKnowBtn" class="btn w-full mt-2" style="background:var(--purple)">ZAPISZ NOTATKĘ</button>
      <div class="small mt-2">Notatki są pogrupowane wg tematu. Pozycje możesz usuwać.</div>
    </div>
    <div id="knowList" class="grid" style="gap:10px"></div>
  </section>
</main>

<!-- MODAL -->
<div id="modalOverlay" class="modal-overlay" aria-hidden="true">
  <div class="modal" role="dialog" aria-modal="true" aria-labelledby="modalTitle">
    <div class="modal-header">
      <div id="modalTitle" class="modal-title">EDYCJA</div>
      <button id="modalClose" class="btn btn-ghost" style="padding:6px 10px">✕</button>
    </div>
    <div id="modalBody" class="modal-body"></div>
    <div class="modal-footer">
      <button id="modalCancel" class="btn btn-slate" style="padding:10px 14px">ANULUJ</button>
      <button id="modalSave" class="btn btn-green" style="padding:10px 14px">ZAPISZ</button>
    </div>
  </div>
</div>

<script>
/* =========================
   Pasiecznik ULTRA – STABILNY JS
   - routing/nav stabilny
   - finanse + magazyn + wiedza
   - przeglądy: dodaj/edytuj/usuń + termin następnego przeglądu (konkretna data)
   - karmienie: historia + edycja/usuwanie
   - matki: historia + aktualna + alarm >24 mies (filtr + czerwone oznaczenie)
   - magazyn: BEZ przycisków -1/+1
========================= */

(() => {
  "use strict";

  /* ===== Helpers ===== */
  const $ = (id) => document.getElementById(id);
  const uid = () =>
    (crypto && crypto.randomUUID) ? crypto.randomUUID() :
      (Date.now().toString(36) + Math.random().toString(36).slice(2));

  const esc = (s) =>
    String(s ?? "")
      .replaceAll("&","&amp;")
      .replaceAll("<","&lt;")
      .replaceAll(">","&gt;")
      .replaceAll('"',"&quot;")
      .replaceAll("'","&#039;");

  const todayISO = () => new Date().toISOString().slice(0,10);

  const isoToPL = (iso) => {
    if (!iso) return "—";
    const [y,m,d] = String(iso).split("-");
    if (!y || !m || !d) return iso;
    return `${d}.${m}.${y}`;
  };

  const parseISODate = (iso) => {
    if (!iso) return null;
    const [y,m,d] = String(iso).split("-").map(Number);
    if (!y || !m || !d) return null;
    return new Date(y, m-1, d, 12, 0, 0, 0);
  };

  const thisYear = new Date().getFullYear();

  function monthsDiffFromYearMonth(y, m, now=new Date()){
    const yy = Number(y), mm = Number(m);
    if (!yy || !mm) return null;
    const nY = now.getFullYear();
    const nM = now.getMonth()+1;
    return (nY-yy)*12 + (nM-mm);
  }

  /* ===== IndexedDB ===== */
  const DB_NAME = "pasiecznik_ultra_db";
  const DB_VER  = 7; // bump -> nowe klucze (feedings, queens)
  let db = null;

  function openDB(){
    return new Promise((resolve, reject) => {
      const req = indexedDB.open(DB_NAME, DB_VER);

      req.onupgradeneeded = (e) => {
        const d = e.target.result;
        if (!d.objectStoreNames.contains("data")) d.createObjectStore("data", { keyPath:"key" });
      };
      req.onsuccess = () => { db = req.result; resolve(db); };
      req.onerror = () => reject(req.error);
    });
  }

  function dbGet(key){
    return new Promise((resolve) => {
      const tx = db.transaction("data","readonly");
      const req = tx.objectStore("data").get(key);
      req.onsuccess = () => resolve(req.result ? req.result.value : null);
      req.onerror = () => resolve(null);
    });
  }

  function dbSet(key, value){
    return new Promise((resolve, reject) => {
      const tx = db.transaction("data","readwrite");
      tx.objectStore("data").put({ key, value });
      tx.oncomplete = () => resolve();
      tx.onerror = () => reject(tx.error);
    });
  }

  /* ===== State ===== */
  const State = {
    view: "hives",
    currentHive: null,

    hives: [], // {id,label,place,kind,year,nextVisitDate}
    reviews: [], // {id,hiveId,visitDate,note,nextVisitDate,ts}

    feedings: [], // {id,hiveId,date,type,qty,unit,note,ts}

    queens: [], // {id,hiveId,year,month,name,insemination,breederLine,opalite,gentleness,note,isCurrent,ts}

    calendar: {}, // { [year]: {0:'',...} }
    finance: [], // {id,type,date,amount,desc}
    invItems: [], // {id,name}
    invMoves: [], // {id,itemId,date,delta,note,ts}
    topics: [],
    knowledge: [] // {id,topic,title,body,dateTS}
  };

  async function loadAll(){
    State.hives     = (await dbGet("hives"))     || [];
    State.reviews   = (await dbGet("reviews"))   || [];
    State.feedings  = (await dbGet("feedings"))  || [];
    State.queens    = (await dbGet("queens"))    || [];
    State.calendar  = (await dbGet("calendar"))  || {};
    State.finance   = (await dbGet("finance"))   || [];
    State.invItems  = (await dbGet("invItems"))  || [];
    State.invMoves  = (await dbGet("invMoves"))  || [];
    State.topics    = (await dbGet("topics"))    || ["Ogólne"];
    State.knowledge = (await dbGet("knowledge")) || [];

    // seed minimal
    if (!State.hives.length){
      State.hives = ["1","2","3"].map(s => ({
        id: s, label: s, place: "", kind: "rodzina", year: thisYear, nextVisitDate: ""
      }));
      await dbSet("hives", State.hives);
    }

    // normalize
    State.hives.forEach(h => {
      if (h.label == null) h.label = h.id;
      if (h.place == null) h.place = "";
      if (h.kind == null) h.kind = "rodzina";
      if (h.year == null) h.year = thisYear;
      if (h.nextVisitDate == null) h.nextVisitDate = "";
    });

    State.reviews.forEach(r => {
      if (!r.id) r.id = uid();
      if (!r.ts) r.ts = Date.now();
      if (r.note == null) r.note = "";
      if (r.nextVisitDate == null) r.nextVisitDate = "";
    });

    State.feedings.forEach(f => {
      if (!f.id) f.id = uid();
      if (!f.ts) f.ts = Date.now();
      if (f.type == null) f.type = "";
      if (f.unit == null) f.unit = "kg";
      if (f.note == null) f.note = "";
    });

    State.queens.forEach(q => {
      if (!q.id) q.id = uid();
      if (!q.ts) q.ts = Date.now();
      if (q.name == null) q.name = "";
      if (q.note == null) q.note = "";
      if (q.breederLine == null) q.breederLine = "";
      if (q.insemination == null) q.insemination = "";
      if (q.opalite == null) q.opalite = "";
      if (q.gentleness == null) q.gentleness = "";
      if (q.isCurrent == null) q.isCurrent = false;
    });
  }

  async function save(key){
    await dbSet(key, State[key]);
  }

  /* ===== Modal ===== */
  const Modal = {
    ctx: null, // {type, id}
    onSave: null
  };

  function modalOpen(title, bodyHTML, onSave){
    $("modalTitle").textContent = title;
    $("modalBody").innerHTML = bodyHTML;
    Modal.onSave = onSave;
    $("modalOverlay").classList.add("show");
    $("modalOverlay").setAttribute("aria-hidden","false");
  }
  function modalClose(){
    $("modalOverlay").classList.remove("show");
    $("modalOverlay").setAttribute("aria-hidden","true");
    $("modalBody").innerHTML = "";
    Modal.onSave = null;
  }

  $("modalClose").addEventListener("click", modalClose);
  $("modalCancel").addEventListener("click", modalClose);
  $("modalOverlay").addEventListener("click", (e)=>{
    if (e.target === $("modalOverlay")) modalClose();
  });
  $("modalSave").addEventListener("click", async ()=>{
    if (typeof Modal.onSave === "function"){
      await Modal.onSave();
    }
  });

  /* ===== Router ===== */
  function hideAll(){
    document.querySelectorAll("main > section").forEach(s => s.classList.add("hidden"));
  }

  function setTitle(){
    const v = State.view;
    $("backBtn").classList.toggle("hidden", v !== "details");
    const map = {
      hives:"PASIECZNIK",
      details:"UL",
      calendar:"KALENDARZ",
      finance:"FINANSE",
      inventory:"MAGAZYN",
      knowledge:"WIEDZA"
    };
    $("appTitle").textContent =
      (v === "details" && State.currentHive) ? ("UL " + State.currentHive) : (map[v] || "PASIECZNIK");
  }

  function renderView(view, hiveId=null, push=true){
    State.view = view;
    State.currentHive = hiveId;

    hideAll();
    const secId = "view" + view.charAt(0).toUpperCase() + view.slice(1);
    const sec = $(secId);
    if (sec) sec.classList.remove("hidden");
    setTitle();

    if (view === "hives") renderHives();
    if (view === "details") renderDetails();
    if (view === "calendar") renderCalendar();
    if (view === "finance") renderFinance();
    if (view === "inventory") renderInventory();
    if (view === "knowledge") renderKnowledge();

    if (push){
      const p = new URLSearchParams();
      p.set("v", view);
      if (view === "details" && hiveId) p.set("id", hiveId);
      history.pushState({view, hiveId}, "", "?" + p.toString());
    }
  }

  window.onpopstate = (e) => {
    const st = e.state;
    if (st && st.view){
      renderView(st.view, st.hiveId || null, false);
      return;
    }
    const p = new URLSearchParams(location.search);
    const v = p.get("v") || "hives";
    const id = p.get("id");
    renderView(v, v === "details" ? id : null, false);
  };

  /* ===== Computed ===== */
  function lastReviewForHive(hiveId){
    const rs = State.reviews
      .filter(r => r.hiveId === hiveId && r.visitDate)
      .sort((a,b) => (b.visitDate||"").localeCompare(a.visitDate||"") || (b.ts||0)-(a.ts||0));
    return rs[0] || null;
  }
  function lastReviewDateForHive(hiveId){
    return lastReviewForHive(hiveId)?.visitDate || "";
  }

  function needsReview(hive){
    const t = todayISO();
    if (hive.nextVisitDate) return hive.nextVisitDate <= t;
    const last = lastReviewDateForHive(hive.id);
    if (!last) return true;
    const lastDt = parseISODate(last);
    const now = parseISODate(t);
    const diffDays = Math.floor((now - lastDt) / (1000*60*60*24));
    return diffDays >= 6;
  }

  function lastFeedingForHive(hiveId){
    return State.feedings
      .filter(f => f.hiveId === hiveId && f.date)
      .sort((a,b) => (b.date||"").localeCompare(a.date||"") || (b.ts||0)-(a.ts||0))[0] || null;
  }

  function currentQueenForHive(hiveId){
    const qs = State.queens.filter(q => q.hiveId === hiveId);
    const current = qs.filter(q => q.isCurrent);
    if (current.length){
      return current.sort((a,b) => (b.ts||0)-(a.ts||0))[0];
    }
    // fallback: najnowsza po roku/miesiącu/ts
    return qs.sort((a,b) => {
      const aKey = (Number(a.year)||0)*100 + (Number(a.month)||0);
      const bKey = (Number(b.year)||0)*100 + (Number(b.month)||0);
      if (bKey !== aKey) return bKey - aKey;
      return (b.ts||0)-(a.ts||0);
    })[0] || null;
  }

  function queenAgeMonths(q){
    if (!q) return null;
    const diff = monthsDiffFromYearMonth(q.year, q.month);
    return (diff == null) ? null : diff;
  }

  function queenNeedsReplace(q){
    const m = queenAgeMonths(q);
    return (m != null && m >= 24);
  }

  /* ===== Filters ===== */
  const Filters = { review:false, queen:false };
  function applyFilterFlags(btn, isOn, style){
    btn.classList.toggle("active", !!isOn);
    if (style==="red") btn.classList.toggle("active-red", !!isOn);
  }
  function renderFilterButtons(){
    applyFilterFlags($("fltReviewBtn"), Filters.review);
    applyFilterFlags($("fltQueenBtn"), Filters.queen, "red");
  }
  function filterHives(list){
    let out = [...list];
    if (Filters.review) out = out.filter(h => needsReview(h));
    if (Filters.queen) out = out.filter(h => queenNeedsReplace(currentQueenForHive(h.id)));
    return out;
  }

  /* ===== Render: Hives ===== */
  function renderHives(){
    renderFilterButtons();
    const grid = $("hivesGrid");
    grid.innerHTML = "";

    const sorted = [...State.hives].sort((a,b) =>
      String(a.label).localeCompare(String(b.label), undefined, {numeric:true})
    );
    const filtered = filterHives(sorted);

    filtered.forEach(h => {
      const due = needsReview(h);
      const lastRev = lastReviewDateForHive(h.id);
      const lastFeed = lastFeedingForHive(h.id);
      const q = currentQueenForHive(h.id);
      const qAge = queenAgeMonths(q);
      const qBad = queenNeedsReplace(q);

      const el = document.createElement("div");
      el.className = "card hive-card";
      el.innerHTML = `
        <div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start">
          <div>
            <div style="font-size:22px;font-weight:1000">${esc(h.label)}</div>
            <div class="small" style="margin-top:6px;display:flex;gap:6px;flex-wrap:wrap">
              <span class="pill">${h.kind==="odklad"?"Odkład":"Rodzina"} ${esc(h.year)}</span>
              ${h.place ? `<span class="pill">📍 ${esc(h.place)}</span>` : ``}
              ${
                h.nextVisitDate
                  ? `<span class="pill ${due?'bad':''}">⏭ ${esc(isoToPL(h.nextVisitDate))}</span>`
                  : (due ? `<span class="pill bad">⏭ BRAK (DO PRZEGLĄDU)</span>` : ``)
              }
            </div>

            <div class="small" style="margin-top:8px;display:flex;gap:6px;flex-wrap:wrap">
              <span class="badge blue">🔎 Ostatni: <b>${lastRev ? isoToPL(lastRev) : "—"}</b></span>
              <span class="badge ${qBad?'red':'green'}">👑 ${q?.name ? esc(q.name) : "brak"}${(qAge!=null)?` • ${qAge} mies.`:""}</span>
              <span class="badge">🍯 Karmienie: <b>${lastFeed?.date ? isoToPL(lastFeed.date) : "—"}</b></span>
            </div>
          </div>

          <div style="text-align:right;min-width:110px">
            <div class="muted">Status</div>
            <div class="${due?'text-alert':''}" style="margin-top:6px;font-weight:1000">
              ${due ? "🔎 DO PRZEGLĄDU" : "✅ OK"}
            </div>
            ${qBad ? `<div class="small" style="margin-top:6px;color:#991b1b;font-weight:1000">MATKA DO WYMIANY</div>` : ``}
          </div>
        </div>
      `;
      el.addEventListener("click", () => renderView("details", h.id, true));
      grid.appendChild(el);
    });

    if (!filtered.length){
      const empty = document.createElement("div");
      empty.className = "card text-center";
      empty.innerHTML = `
        <div style="font-weight:1000">Brak uli w tym filtrze.</div>
        <div class="small mt-2">Kliknij „Wyczyść”, żeby zobaczyć wszystkie.</div>
      `;
      grid.appendChild(empty);
    }
  }

  /* ===== Details ===== */
  function fillQueenYearMonthSelects(){
    const ys = $("qYear");
    const ms = $("qMonth");
    if (!ys.options.length){
      for (let y=thisYear-5; y<=thisYear+1; y++){
        const o=document.createElement("option");
        o.value=String(y); o.textContent=String(y);
        ys.appendChild(o);
      }
    }
    if (!ms.options.length){
      const names = ["01","02","03","04","05","06","07","08","09","10","11","12"];
      names.forEach((n,i)=>{
        const o=document.createElement("option");
        o.value=String(i+1); o.textContent=n;
        ms.appendChild(o);
      });
    }
    ys.value = ys.value || String(thisYear);
    ms.value = ms.value || String(new Date().getMonth()+1);
  }

  function renderDetails(){
    const hiveId = State.currentHive;
    const hive = State.hives.find(h => h.id === hiveId);
    if (!hive){
      renderView("hives", null, true);
      return;
    }

    $("detailsTitle").textContent = `${hive.label} • ${hive.kind==="odklad"?"Odkład":"Rodzina"} • ${hive.year}`;

    $("hLabel").value = hive.label || hive.id || "";
    $("hPlace").value = hive.place || "";
    $("hType").value  = hive.kind || "rodzina";
    $("hYear").value  = hive.year || thisYear;
    $("hNextVisit").value = hive.nextVisitDate || "";

    // defaults for forms
    $("revDate").value = $("revDate").value || todayISO();
    $("revNext").value = $("revNext").value || hive.nextVisitDate || "";
    $("feedDate").value = $("feedDate").value || todayISO();

    fillQueenYearMonthSelects();

    renderReviews();
    renderFeedings();
    renderQueens();
  }

  async function saveHiveMeta(){
    const oldId = State.currentHive;
    const hive = State.hives.find(h => h.id === oldId);
    if (!hive) return;

    const newLabel = ($("hLabel").value || "").trim();
    const newPlace = ($("hPlace").value || "").trim();
    const newNext  = ($("hNextVisit").value || "").trim();

    if (!newLabel) return alert("Numer / nazwa ula nie może być pusta.");

    const newId = newLabel;
    if (newId !== oldId && State.hives.some(h => h.id === newId)){
      return alert("Taki numer/nazwa ula już istnieje. Wpisz inny.");
    }

    hive.kind  = $("hType").value;
    hive.year  = Number($("hYear").value) || thisYear;
    hive.place = newPlace;
    hive.nextVisitDate = newNext;

    if (newId !== oldId){
      // zmiana ID -> przenieś powiązane rekordy
      hive.id = newId;
      hive.label = newLabel;

      State.reviews.forEach(r => { if (r.hiveId === oldId) r.hiveId = newId; });
      State.feedings.forEach(f => { if (f.hiveId === oldId) f.hiveId = newId; });
      State.queens.forEach(q => { if (q.hiveId === oldId) q.hiveId = newId; });

      await save("reviews");
      await save("feedings");
      await save("queens");

      State.currentHive = newId;
      const p = new URLSearchParams(location.search);
      p.set("v","details"); p.set("id", newId);
      history.replaceState({view:"details", hiveId:newId}, "", "?" + p.toString());
    } else {
      hive.label = newLabel;
    }

    await save("hives");
    renderDetails();
    renderHives();
  }

  /* ===== Reviews ===== */
  function renderReviews(){
    const hiveId = State.currentHive;
    const list = $("revList");
    list.innerHTML = "";

    const items = State.reviews
      .filter(r => r.hiveId === hiveId)
      .sort((a,b) => (b.visitDate||"").localeCompare(a.visitDate||"") || (b.ts||0)-(a.ts||0));

    if (!items.length){
      list.innerHTML = `<div class="card text-center"><div style="font-weight:1000">Brak przeglądów.</div><div class="small mt-2">Dodaj pierwszy przegląd powyżej.</div></div>`;
      return;
    }

    items.forEach(r => {
      const el = document.createElement("div");
      el.className = "card";
      const next = r.nextVisitDate ? isoToPL(r.nextVisitDate) : "—";
      el.innerHTML = `
        <div class="kv">
          <div>
            <div style="font-weight:1000">📅 ${isoToPL(r.visitDate)} <span class="small"> • następny: <b>${next}</b></span></div>
            <div style="white-space:pre-wrap;margin-top:6px">${esc(r.note||"") || "<span class='small'>—</span>"}</div>
          </div>
          <div class="flex gap-1" style="justify-content:flex-end;flex-wrap:wrap">
            <button class="btn btn-blue tinybtn" data-act="editReview" data-id="${esc(r.id)}">EDYTUJ</button>
            <button class="btn btn-slate tinybtn" data-act="delReview" data-id="${esc(r.id)}">USUŃ</button>
          </div>
        </div>
      `;
      list.appendChild(el);
    });
  }

  async function addReview(){
    const hiveId = State.currentHive;
    const visitDate = ($("revDate").value || "").trim();
    const note = ($("revNote").value || "").trim();
    const nextVisitDate = ($("revNext").value || "").trim();

    if (!visitDate) return alert("Podaj datę przeglądu.");

    State.reviews.push({
      id: uid(),
      hiveId,
      visitDate,
      note,
      nextVisitDate,
      ts: Date.now()
    });

    // opcjonalnie: od razu ustaw w ulu termin kolejnego przeglądu
    const hive = State.hives.find(h => h.id === hiveId);
    if (hive && nextVisitDate){
      hive.nextVisitDate = nextVisitDate;
      await save("hives");
      $("hNextVisit").value = nextVisitDate;
    }

    await save("reviews");

    $("revNote").value = "";
    // zostaw daty, żeby łatwo dodawać kolejne
    renderReviews();
    renderHives();
  }

  function editReviewModal(id){
    const r = State.reviews.find(x => x.id === id);
    if (!r) return;

    modalOpen("Edycja przeglądu", `
      <div class="row cols2">
        <label class="help">Data przeglądu
          <input id="mRevDate" type="date" class="input" value="${esc(r.visitDate||"")}">
        </label>
        <label class="help">Termin następnego przeglądu
          <input id="mRevNext" type="date" class="input" value="${esc(r.nextVisitDate||"")}">
        </label>
      </div>
      <label class="help">Notatka
        <textarea id="mRevNote" class="input" style="height:120px">${esc(r.note||"")}</textarea>
      </label>
      <div class="small">Możesz edytować zarówno datę, jak i treść.</div>
    `, async ()=>{
      const d = ($("mRevDate").value || "").trim();
      if (!d) return alert("Data przeglądu jest wymagana.");

      r.visitDate = d;
      r.nextVisitDate = ($("mRevNext").value || "").trim();
      r.note = ($("mRevNote").value || "").trim();
      r.ts = Date.now();

      await save("reviews");

      // jeśli edytujesz termin następnego – aktualizuj ul (opcjonalnie)
      const hive = State.hives.find(h => h.id === r.hiveId);
      if (hive && r.nextVisitDate){
        hive.nextVisitDate = r.nextVisitDate;
        await save("hives");
        if (State.view === "details") $("hNextVisit").value = hive.nextVisitDate || "";
      }

      modalClose();
      renderReviews();
      renderHives();
    });
  }

  async function deleteReview(id){
    if (!confirm("Usunąć przegląd?")) return;
    State.reviews = State.reviews.filter(x => x.id !== id);
    await save("reviews");
    renderReviews();
    renderHives();
  }

  /* ===== Feedings ===== */
  function renderFeedings(){
    const hiveId = State.currentHive;
    const list = $("feedList");
    list.innerHTML = "";

    const items = State.feedings
      .filter(f => f.hiveId === hiveId)
      .sort((a,b) => (b.date||"").localeCompare(a.date||"") || (b.ts||0)-(a.ts||0));

    if (!items.length){
      list.innerHTML = `<div class="card text-center"><div style="font-weight:1000">Brak karmień.</div><div class="small mt-2">Dodaj pierwsze karmienie powyżej.</div></div>`;
      return;
    }

    items.forEach(f => {
      const el = document.createElement("div");
      el.className = "card";
      el.innerHTML = `
        <div class="kv">
          <div>
            <div style="font-weight:1000">🍯 ${isoToPL(f.date)} • ${esc(f.type||"—")} • <b>${Number(f.qty||0).toFixed(2)}</b> ${esc(f.unit||"")}</div>
            ${f.note ? `<div class="small" style="margin-top:6px">${esc(f.note)}</div>` : `<div class="small" style="margin-top:6px">—</div>`}
          </div>
          <div class="flex gap-1" style="justify-content:flex-end;flex-wrap:wrap">
            <button class="btn btn-blue tinybtn" data-act="editFeeding" data-id="${esc(f.id)}">EDYTUJ</button>
            <button class="btn btn-slate tinybtn" data-act="delFeeding" data-id="${esc(f.id)}">USUŃ</button>
          </div>
        </div>
      `;
      list.appendChild(el);
    });
  }

  async function addFeeding(){
    const hiveId = State.currentHive;
    const date = ($("feedDate").value || "").trim();
    const type = ($("feedType").value || "").trim();
    const qty = Number($("feedQty").value);
    const unit = ($("feedUnit").value || "kg").trim();
    const note = ($("feedNote").value || "").trim();

    if (!date) return alert("Podaj datę karmienia.");
    if (!type) return alert("Podaj rodzaj pokarmu.");
    if (!Number.isFinite(qty) || qty <= 0) return alert("Podaj ilość > 0.");

    State.feedings.push({ id:uid(), hiveId, date, type, qty, unit, note, ts:Date.now() });
    await save("feedings");

    $("feedType").value = "";
    $("feedQty").value = "";
    $("feedNote").value = "";
    renderFeedings();
    renderHives();
  }

  function editFeedingModal(id){
    const f = State.feedings.find(x => x.id === id);
    if (!f) return;

    modalOpen("Edycja karmienia", `
      <div class="row cols2">
        <label class="help">Data
          <input id="mFeedDate" type="date" class="input" value="${esc(f.date||"")}">
        </label>
        <label class="help">Rodzaj pokarmu
          <input id="mFeedType" type="text" class="input" value="${esc(f.type||"")}">
        </label>
      </div>
      <div class="row cols2">
        <label class="help">Ilość
          <input id="mFeedQty" type="number" step="0.01" class="input" value="${esc(f.qty ?? "")}">
        </label>
        <label class="help">Jednostka
          <select id="mFeedUnit" class="input">
            <option value="kg" ${f.unit==="kg"?"selected":""}>kg</option>
            <option value="l" ${f.unit==="l"?"selected":""}>l</option>
          </select>
        </label>
      </div>
      <label class="help">Notatka
        <input id="mFeedNote" type="text" class="input" value="${esc(f.note||"")}">
      </label>
    `, async ()=>{
      const d = ($("mFeedDate").value || "").trim();
      const t = ($("mFeedType").value || "").trim();
      const q = Number($("mFeedQty").value);
      if (!d) return alert("Data jest wymagana.");
      if (!t) return alert("Rodzaj jest wymagany.");
      if (!Number.isFinite(q) || q<=0) return alert("Ilość > 0.");

      f.date = d;
      f.type = t;
      f.qty = q;
      f.unit = ($("mFeedUnit").value || "kg").trim();
      f.note = ($("mFeedNote").value || "").trim();
      f.ts = Date.now();

      await save("feedings");
      modalClose();
      renderFeedings();
      renderHives();
    });
  }

  async function deleteFeeding(id){
    if (!confirm("Usunąć wpis karmienia?")) return;
    State.feedings = State.feedings.filter(x => x.id !== id);
    await save("feedings");
    renderFeedings();
    renderHives();
  }

  /* ===== Queens ===== */
  function renderQueens(){
    const hiveId = State.currentHive;
    const list = $("queenList");
    list.innerHTML = "";

    const qs = State.queens
      .filter(q => q.hiveId === hiveId)
      .sort((a,b) => {
        const aKey = (Number(a.year)||0)*100 + (Number(a.month)||0);
        const bKey = (Number(b.year)||0)*100 + (Number(b.month)||0);
        if (bKey !== aKey) return bKey - aKey;
        return (b.ts||0)-(a.ts||0);
      });

    const cur = currentQueenForHive(hiveId);
    if (!cur){
      $("queenStatus").innerHTML = `Aktualna matka: <b>brak</b>.`;
    } else {
      const age = queenAgeMonths(cur);
      const bad = queenNeedsReplace(cur);
      $("queenStatus").innerHTML = `Aktualna matka: <b>${esc(cur.name||"") || "—"}</b> • rocznik: <b>${esc(cur.year)}-${String(cur.month).padStart(2,"0")}</b> • wiek: <b>${age!=null?age+" mies.":"—"}</b> ${bad?`<span style="color:#991b1b;font-weight:1000">• MATKA DO WYMIANY</span>`:""}`;
    }

    if (!qs.length){
      list.innerHTML = `<div class="card text-center"><div style="font-weight:1000">Brak historii matek.</div><div class="small mt-2">Dodaj pierwszą matkę powyżej.</div></div>`;
      return;
    }

    qs.forEach(q => {
      const age = queenAgeMonths(q);
      const bad = queenNeedsReplace(q);
      const isCur = !!q.isCurrent || (cur && cur.id === q.id);

      const el = document.createElement("div");
      el.className = "card";
      el.style.borderLeft = "4px solid " + (bad ? "var(--danger)" : (isCur ? "var(--purple)" : "var(--border)"));

      el.innerHTML = `
        <div class="kv">
          <div>
            <div style="font-weight:1000">
              👑 ${esc(q.name||"(bez nazwy)")}
              ${isCur ? `<span class="badge blue" style="margin-left:6px">AKTUALNA</span>` : ``}
              ${bad ? `<span class="badge red" style="margin-left:6px">MATKA DO WYMIANY</span>` : ``}
            </div>
            <div class="small" style="margin-top:6px">
              Rocznik: <b>${esc(q.year)}-${String(q.month).padStart(2,"0")}</b>
              ${age!=null ? ` • Wiek: <b>${age} mies.</b>` : ``}
              ${q.insemination ? ` • Unasiennianie: <b>${esc(q.insemination)}</b>` : ``}
              ${q.breederLine ? ` • Linia/Hodowca: <b>${esc(q.breederLine)}</b>` : ``}
            </div>
            <div class="small" style="margin-top:6px">
              ${q.opalite ? `Opalitka: <b>${esc(q.opalite)}</b>` : `Opalitka: <b>—</b>`}
              ${q.gentleness ? ` • Łagodność: <b>${esc(q.gentleness)}/5</b>` : ``}
            </div>
            ${q.note ? `<div class="small" style="margin-top:8px;white-space:pre-wrap">${esc(q.note)}</div>` : `<div class="small" style="margin-top:8px">—</div>`}
          </div>

          <div class="flex gap-1" style="justify-content:flex-end;flex-wrap:wrap">
            <button class="btn btn-orange tinybtn" data-act="setCurrentQueen" data-id="${esc(q.id)}">USTAW AKT.</button>
            <button class="btn btn-blue tinybtn" data-act="editQueen" data-id="${esc(q.id)}">EDYTUJ</button>
            <button class="btn btn-slate tinybtn" data-act="delQueen" data-id="${esc(q.id)}">USUŃ</button>
          </div>
        </div>
      `;

      list.appendChild(el);
    });
  }

  async function addQueen(){
    const hiveId = State.currentHive;
    const name = ($("qName").value || "").trim();
    const breederLine = ($("qBreederLine").value || "").trim();
    const year = Number(($("qYear").value || "").trim());
    const month = Number(($("qMonth").value || "").trim());
    const insemination = ($("qInsem").value || "").trim();
    const opalite = ($("qOpalit").value || "").trim();
    const gentleness = ($("qGentle").value || "").trim();
    const note = ($("qNote").value || "").trim();
    const isCurrent = !!$("qIsCurrent").checked;

    if (!name) return alert("Podaj nazwę matki.");
    if (!year || !month) return alert("Podaj rocznik i miesiąc.");

    const rec = {
      id: uid(), hiveId,
      year, month,
      name,
      insemination,
      breederLine,
      opalite,
      gentleness,
      note,
      isCurrent,
      ts: Date.now()
    };

    if (isCurrent){
      // zdejmij aktualną z pozostałych w ulu
      State.queens.forEach(q => {
        if (q.hiveId === hiveId) q.isCurrent = false;
      });
    }

    State.queens.push(rec);
    await save("queens");

    $("qName").value = "";
    $("qBreederLine").value = "";
    $("qInsem").value = "";
    $("qOpalit").value = "";
    $("qGentle").value = "";
    $("qNote").value = "";
    $("qIsCurrent").checked = false;

    renderQueens();
    renderHives();
  }

  async function setCurrentQueen(id){
    const hiveId = State.currentHive;
    const q = State.queens.find(x => x.id === id);
    if (!q) return;

    State.queens.forEach(x => {
      if (x.hiveId === hiveId) x.isCurrent = false;
    });
    q.isCurrent = true;
    q.ts = Date.now();

    await save("queens");
    renderQueens();
    renderHives();
  }

  function editQueenModal(id){
    const q = State.queens.find(x => x.id === id);
    if (!q) return;

    modalOpen("Edycja matki", `
      <div class="row cols2">
        <label class="help">Nazwa
          <input id="mQName" type="text" class="input" value="${esc(q.name||"")}">
        </label>
        <label class="help">Linia / hodowca
          <input id="mQBreeder" type="text" class="input" value="${esc(q.breederLine||"")}">
        </label>
      </div>

      <div class="row cols2">
        <label class="help">Rok
          <input id="mQYear" type="number" class="input" value="${esc(q.year||"")}">
        </label>
        <label class="help">Miesiąc
          <input id="mQMonth" type="number" min="1" max="12" class="input" value="${esc(q.month||"")}">
        </label>
      </div>

      <div class="row cols2">
        <label class="help">Typ unasienniania
          <select id="mQInsem" class="input">
            <option value="" ${q.insemination===""?"selected":""}>—</option>
            <option value="naturalne" ${q.insemination==="naturalne"?"selected":""}>Naturalne</option>
            <option value="sztuczne" ${q.insemination==="sztuczne"?"selected":""}>Sztuczne (AI)</option>
            <option value="nieznane" ${q.insemination==="nieznane"?"selected":""}>Nieznane</option>
          </select>
        </label>
        <label class="help">Kolor opalitki
          <select id="mQOpalit" class="input">
            <option value="" ${q.opalite===""?"selected":""}>—</option>
            <option value="brak" ${q.opalite==="brak"?"selected":""}>Brak opalitki</option>
            <option value="biała" ${q.opalite==="biała"?"selected":""}>Biała</option>
            <option value="żółta" ${q.opalite==="żółta"?"selected":""}>Żółta</option>
            <option value="czerwona" ${q.opalite==="czerwona"?"selected":""}>Czerwona</option>
            <option value="zielona" ${q.opalite==="zielona"?"selected":""}>Zielona</option>
            <option value="niebieska" ${q.opalite==="niebieska"?"selected":""}>Niebieska</option>
          </select>
        </label>
      </div>

      <div class="row cols2">
        <label class="help">Łagodność
          <select id="mQGentle" class="input">
            <option value="" ${q.gentleness===""?"selected":""}>—</option>
            <option value="1" ${q.gentleness==="1"?"selected":""}>1</option>
            <option value="2" ${q.gentleness==="2"?"selected":""}>2</option>
            <option value="3" ${q.gentleness==="3"?"selected":""}>3</option>
            <option value="4" ${q.gentleness==="4"?"selected":""}>4</option>
            <option value="5" ${q.gentleness==="5"?"selected":""}>5</option>
          </select>
        </label>
        <label class="help" style="display:flex;align-items:center;gap:10px;cursor:pointer">
          <input id="mQCurrent" type="checkbox" style="width:18px;height:18px" ${q.isCurrent?"checked":""}>
          Ustaw jako <b>aktualną</b>
        </label>
      </div>

      <label class="help">Wolna notatka
        <textarea id="mQNote" class="input" style="height:120px">${esc(q.note||"")}</textarea>
      </label>
    `, async ()=>{
      const name = ($("mQName").value || "").trim();
      const breederLine = ($("mQBreeder").value || "").trim();
      const year = Number(($("mQYear").value || "").trim());
      const month = Number(($("mQMonth").value || "").trim());
      const insemination = ($("mQInsem").value || "").trim();
      const opalite = ($("mQOpalit").value || "").trim();
      const gentleness = ($("mQGentle").value || "").trim();
      const note = ($("mQNote").value || "").trim();
      const isCurrent = !!$("mQCurrent").checked;

      if (!name) return alert("Nazwa matki jest wymagana.");
      if (!year || !month || month<1 || month>12) return alert("Rok/miesiąc są wymagane (miesiąc 1-12).");

      q.name = name;
      q.breederLine = breederLine;
      q.year = year;
      q.month = month;
      q.insemination = insemination;
      q.opalite = opalite;
      q.gentleness = gentleness;
      q.note = note;
      q.ts = Date.now();

      if (isCurrent){
        State.queens.forEach(x => {
          if (x.hiveId === q.hiveId) x.isCurrent = false;
        });
        q.isCurrent = true;
      } else {
        q.isCurrent = false;
      }

      await save("queens");
      modalClose();
      renderQueens();
      renderHives();
    });
  }

  async function deleteQueen(id){
    if (!confirm("Usunąć wpis matki?")) return;
    State.queens = State.queens.filter(x => x.id !== id);
    await save("queens");
    renderQueens();
    renderHives();
  }

  /* ===== Calendar ===== */
  function ensureCalendarYear(y){
    if (!State.calendar[y]) State.calendar[y] = {};
  }

  function renderCalendar(){
    const months = ["Styczeń","Luty","Marzec","Kwiecień","Maj","Czerwiec","Lipiec","Sierpień","Wrzesień","Październik","Listopad","Grudzień"];
    const yearSel = $("calYear");

    if (!yearSel.options.length){
      for (let y=2022;y<=2027;y++){
        const o=document.createElement("option");
        o.value=String(y); o.textContent=String(y);
        yearSel.appendChild(o);
      }
    }

    const current = yearSel.value || String(thisYear);
    yearSel.value = (Number(current)>=2022 && Number(current)<=2027) ? current : String(thisYear);

    ensureCalendarYear(yearSel.value);

    const grid = $("calendarGrid");
    grid.innerHTML = "";

    months.forEach((m,idx) => {
      const card = document.createElement("div");
      card.className = "card";
      card.innerHTML = `<div style="font-weight:1000;color:#4f46e5;margin-bottom:6px">${m} ${yearSel.value}</div>`;
      const area = document.createElement("textarea");
      area.className="input";
      area.style.height="75px";
      area.value = State.calendar[yearSel.value][idx] || "";
      area.addEventListener("change", async (e) => {
        ensureCalendarYear(yearSel.value);
        State.calendar[yearSel.value][idx] = e.target.value;
        await save("calendar");
      });
      card.appendChild(area);
      grid.appendChild(card);
    });

    yearSel.onchange = () => renderCalendar();
  }

  /* ===== Finance ===== */
  function renderFinance(){
    const list = $("finList");
    list.innerHTML = "";

    let inc=0, exp=0;
    const items = [...State.finance].sort((a,b) => (b.date||"").localeCompare(a.date||""));

    items.forEach(f => {
      if (f.type === "income") inc += Number(f.amount||0);
      else exp += Number(f.amount||0);

      const el = document.createElement("div");
      el.className = "card";
      el.style.borderLeft = "4px solid " + (f.type==="income" ? "var(--success)" : "var(--danger)");
      el.innerHTML = `
        <div class="flex justify-between items-center">
          <div>
            <div style="font-weight:1000">${esc(f.desc)}</div>
            <div class="small">${isoToPL(f.date)} • ${f.type==="income"?"Przychód":"Wydatek"}</div>
          </div>
          <div style="text-align:right">
            <div style="font-weight:1000;color:${f.type==="income"?"var(--success)":"var(--danger)"}">
              ${f.type==="income"?"+":"-"}${Number(f.amount||0).toFixed(2)} zł
            </div>
            <div class="flex gap-2" style="justify-content:flex-end;margin-top:8px">
              <button class="btn btn-slate tinybtn" data-del="fin" data-id="${esc(f.id)}">USUŃ</button>
            </div>
          </div>
        </div>
      `;
      list.appendChild(el);
    });

    $("finInc").textContent = inc.toFixed(2);
    $("finExp").textContent = exp.toFixed(2);
    const bal = inc-exp;
    $("finBalance").textContent = bal.toFixed(2) + " PLN";
    $("finBalance").style.color = bal>=0 ? "var(--success)" : "var(--danger)";
  }

  async function addFinance(){
    const type = $("finType").value;
    const date = $("finDate").value || todayISO();
    const amount = Number($("finAmount").value);
    const desc = ($("finDesc").value || "").trim();

    if (!desc || !Number.isFinite(amount) || amount<=0){
      return alert("Podaj opis i kwotę > 0.");
    }

    State.finance.push({ id:uid(), type, date, amount, desc });
    await save("finance");

    $("finAmount").value = "";
    $("finDesc").value = "";
    renderFinance();
  }

  /* ===== Inventory ===== */
  function invCalcQtyAtDate(itemId, isoDate){
    const d = isoDate || todayISO();
    return State.invMoves
      .filter(m => m.itemId === itemId && (m.date||"") <= d)
      .reduce((s,m) => s + (Number(m.delta)||0), 0);
  }

  function invCalcCurrentQty(itemId){
    return State.invMoves
      .filter(m => m.itemId === itemId)
      .reduce((s,m) => s + (Number(m.delta)||0), 0);
  }

  function refreshInvSelectors(){
    const items = [...State.invItems].sort((a,b) => String(a.name).localeCompare(String(b.name)));
    const sel  = $("invItemSel");
    const qsel = $("invQueryItem");

    sel.innerHTML  = `<option value="">Wybierz produkt…</option>`;
    qsel.innerHTML = `<option value="">Wybierz produkt…</option>`;

    items.forEach(it => {
      const o=document.createElement("option");
      o.value=it.id; o.textContent=it.name;
      sel.appendChild(o);

      const o2=document.createElement("option");
      o2.value=it.id; o2.textContent=it.name;
      qsel.appendChild(o2);
    });
  }

  function renderInventory(){
    refreshInvSelectors();
    $("invMoveDate").value  = $("invMoveDate").value  || todayISO();
    $("invQueryDate").value = $("invQueryDate").value || todayISO();

    const list = $("invList");
    list.innerHTML = "";

    const items = [...State.invItems].sort((a,b) => String(a.name).localeCompare(String(b.name)));
    if (!items.length){
      list.innerHTML = `
        <div class="card text-center">
          <div style="font-weight:1000">Brak produktów w magazynie.</div>
          <div class="small mt-2">Dodaj pierwszy ruch (np. zakup).</div>
        </div>
      `;
      return;
    }

    for (const it of items){
      const qty = invCalcCurrentQty(it.id);
      const el = document.createElement("div");
      el.className = "card";
      el.style.borderLeft = "4px solid var(--slate)";

      const lastMoves = State.invMoves
        .filter(m => m.itemId === it.id)
        .sort((a,b) => (b.date||"").localeCompare(a.date||"") || (b.ts||0)-(a.ts||0))
        .slice(0,5);

      el.innerHTML = `
        <div class="flex justify-between items-center" style="gap:10px">
          <div style="flex:1">
            <div style="font-weight:1000">${esc(it.name)}</div>
            <div class="small">Aktualnie: <b>${qty.toFixed(2)}</b></div>
          </div>
          <div class="flex gap-2 items-center" style="flex-wrap:wrap;justify-content:flex-end">
            <button class="btn btn-slate tinybtn" data-del="invitem" data-id="${esc(it.id)}">USUŃ</button>
          </div>
        </div>
        <div class="hr"></div>
        <div class="small">Ostatnie ruchy:</div>
        <div style="display:grid;gap:6px;margin-top:8px">
          ${
            lastMoves.length ? lastMoves.map(m => `
              <div class="flex justify-between items-center" style="gap:10px">
                <div class="small" style="flex:1"><b>${isoToPL(m.date)}</b> • ${esc(m.note||"—")}</div>
                <div style="font-weight:1000;color:${Number(m.delta)>=0?'var(--success)':'var(--danger)'}">
                  ${Number(m.delta)>=0?'+':''}${Number(m.delta||0).toFixed(2)}
                </div>
                <button class="btn btn-slate tinybtn" data-del="invmove" data-id="${esc(m.id)}">✕</button>
              </div>
            `).join("") : `<div class="small">Brak ruchów.</div>`
          }
        </div>
      `;
      list.appendChild(el);
    }
  }

  async function invAddMove(){
    const selId  = $("invItemSel").value;
    const newName = ($("invNewName").value || "").trim();
    const date   = $("invMoveDate").value || todayISO();
    const delta  = Number($("invMoveDelta").value);
    const note   = ($("invMoveNote").value || "").trim();

    if (!Number.isFinite(delta) || delta===0) return alert("Podaj ilość (różną od 0).");
    if (!date) return alert("Podaj datę.");

    let itemId = selId;

    if (!itemId){
      if (!newName) return alert("Wybierz produkt lub wpisz nowy.");
      const ex = State.invItems.find(x => String(x.name).toLowerCase() === newName.toLowerCase());
      if (ex) itemId = ex.id;
      else {
        itemId = uid();
        State.invItems.push({ id:itemId, name:newName });
        await save("invItems");
      }
    }

    State.invMoves.push({ id:uid(), itemId, date, delta, note, ts:Date.now() });
    await save("invMoves");

    $("invMoveDelta").value = "";
    $("invMoveNote").value = "";
    $("invNewName").value = "";
    $("invItemSel").value = "";

    renderInventory();
  }

  function invQuery(){
    const itemId = $("invQueryItem").value;
    const date = $("invQueryDate").value || todayISO();
    const box = $("invQueryResult");
    box.innerHTML = "";

    if (!itemId) return alert("Wybierz produkt do sprawdzenia.");

    const item = State.invItems.find(x => x.id === itemId);
    const qty = invCalcQtyAtDate(itemId, date);

    const moves = State.invMoves
      .filter(m => m.itemId === itemId && (m.date||"") <= date)
      .sort((a,b) => (b.date||"").localeCompare(a.date||"") || (b.ts||0)-(a.ts||0))
      .slice(0,25);

    box.innerHTML = `
      <div class="card" style="border-left:4px solid var(--blue);padding:12px">
        <div class="muted">WYNIK</div>
        <div style="font-weight:1000;font-size:20px;margin-top:6px">${esc(item?.name || "")}</div>
        <div class="small" style="margin-top:6px">Stan na dzień <b>${isoToPL(date)}</b>:</div>
        <div style="font-weight:1000;font-size:26px;margin-top:6px;color:var(--blue)">${qty.toFixed(2)}</div>
        <div class="hr"></div>
        <div class="small"><b>Ruchy do tego dnia</b> (ostatnie 25):</div>
        <div style="display:grid;gap:6px;margin-top:8px">
          ${
            moves.length ? moves.map(m => `
              <div class="flex justify-between items-center" style="gap:10px">
                <div class="small" style="flex:1"><b>${isoToPL(m.date)}</b> • ${esc(m.note||"—")}</div>
                <div style="font-weight:1000;color:${Number(m.delta)>=0?'var(--success)':'var(--danger)'}">
                  ${Number(m.delta)>=0?'+':''}${Number(m.delta||0).toFixed(2)}
                </div>
              </div>
            `).join("") : `<div class="small">Brak ruchów do tego dnia.</div>`
          }
        </div>
      </div>
    `;
  }

  /* ===== Knowledge ===== */
  function refreshTopicSelect(){
    const sel = $("knowTopic");
    sel.innerHTML = "";
    (State.topics.length ? State.topics : ["Ogólne"]).forEach(t => {
      const o=document.createElement("option");
      o.value=t; o.textContent=t;
      sel.appendChild(o);
    });
  }

  function renderKnowledge(){
    refreshTopicSelect();
    const wrap = $("knowList");
    wrap.innerHTML = "";

    const byTopic = {};
    for (const n of State.knowledge){
      const t = n.topic || "Ogólne";
      if (!byTopic[t]) byTopic[t]=[];
      byTopic[t].push(n);
    }

    const topics = [...new Set([...State.topics, ...Object.keys(byTopic)])].filter(Boolean);

    topics.forEach(t => {
      const items = (byTopic[t] || []).sort((a,b) => (b.dateTS||0)-(a.dateTS||0));
      const sec = document.createElement("div");
      sec.className = "card";
      sec.style.borderLeft = "4px solid var(--purple)";
      sec.innerHTML = `
        <div style="display:flex;justify-content:space-between;align-items:center;gap:10px">
          <div style="font-weight:1000;color:var(--purple)">${esc(t)}</div>
          <div class="small">${items.length} not.</div>
        </div>
      `;

      if (!items.length){
        const p=document.createElement("div");
        p.className="small";
        p.style.marginTop="8px";
        p.textContent="Brak notatek w tym temacie.";
        sec.appendChild(p);
      } else {
        items.forEach(n => {
          const item=document.createElement("div");
          item.style.marginTop="10px";
          item.style.paddingTop="10px";
          item.style.borderTop="1px solid var(--border)";
          item.innerHTML = `
            <div class="flex justify-between items-center">
              <div>
                <div style="font-weight:1000">${esc(n.title||"(bez tytułu)")}</div>
                <div class="small">${new Date(n.dateTS||Date.now()).toLocaleDateString("pl-PL")}</div>
              </div>
              <div class="flex gap-2">
                <button class="btn btn-slate tinybtn" data-del="know" data-id="${esc(n.id)}">USUŃ</button>
              </div>
            </div>
            <div style="white-space:pre-wrap;margin-top:6px">${esc(n.body||"")}</div>
          `;
          sec.appendChild(item);
        });
      }

      wrap.appendChild(sec);
    });
  }

  async function addTopic(){
    const t = ($("knowNewTopic").value || "").trim();
    if (!t) return;
    if (!State.topics.includes(t)) State.topics.push(t);
    await save("topics");
    $("knowNewTopic").value = "";
    renderKnowledge();
  }

  async function addKnowledge(){
    const topic = $("knowTopic").value || "Ogólne";
    const title = ($("knowTitle").value || "").trim();
    const body  = ($("knowBody").value || "").trim();
    if (!title && !body) return alert("Wpisz tytuł lub treść.");

    State.knowledge.push({ id:uid(), topic, title:title||"(bez tytułu)", body, dateTS:Date.now() });
    await save("knowledge");

    $("knowTitle").value = "";
    $("knowBody").value = "";
    renderKnowledge();
  }

  /* ===== Export / Import ===== */
  async function exportBackup(){
    const out = {
      meta:{ app:"Pasiecznik ULTRA", exportedAt:new Date().toISOString() },
      data:{
        hives: State.hives,
        reviews: State.reviews,
        feedings: State.feedings,
        queens: State.queens,
        calendar: State.calendar,
        finance: State.finance,
        invItems: State.invItems,
        invMoves: State.invMoves,
        topics: State.topics,
        knowledge: State.knowledge
      }
    };

    const blob = new Blob([JSON.stringify(out, null, 2)], {type:"application/json"});
    const a = document.createElement("a");
    a.href = URL.createObjectURL(blob);
    a.download = "pasiecznik_backup_" + new Date().toLocaleDateString("pl-PL") + ".json";
    a.click();
  }

  async function importBackup(file){
    const txt = await file.text();
    let parsed;
    try { parsed = JSON.parse(txt); } catch { return alert("Błędny plik."); }

    if (!confirm("Nadpisać wszystkie dane w aplikacji?")) return;

    const data = parsed.data || parsed;

    State.hives     = data.hives     || [];
    State.reviews   = data.reviews   || [];
    State.feedings  = data.feedings  || [];
    State.queens    = data.queens    || [];
    State.calendar  = data.calendar  || {};
    State.finance   = data.finance   || [];
    State.invItems  = data.invItems  || [];
    State.invMoves  = data.invMoves  || [];
    State.topics    = data.topics    || ["Ogólne"];
    State.knowledge = data.knowledge || [];

    // normalize minimal
    State.hives.forEach(h => {
      if (h.label == null) h.label = h.id;
      if (h.place == null) h.place = "";
      if (h.kind == null) h.kind = "rodzina";
      if (h.year == null) h.year = thisYear;
      if (h.nextVisitDate == null) h.nextVisitDate = "";
    });

    await dbSet("hives", State.hives);
    await dbSet("reviews", State.reviews);
    await dbSet("feedings", State.feedings);
    await dbSet("queens", State.queens);
    await dbSet("calendar", State.calendar);
    await dbSet("finance", State.finance);
    await dbSet("invItems", State.invItems);
    await dbSet("invMoves", State.invMoves);
    await dbSet("topics", State.topics);
    await dbSet("knowledge", State.knowledge);

    alert("Przywrócono dane.");
    renderView("hives", null, true);
  }

  /* ===== Add Hive ===== */
  async function addHive(){
    const label = prompt("Numer/Nazwa ula (np. 12, A1):");
    if (!label) return;

    const place = prompt("Położenie ula (opcjonalnie, np. Rząd 2 / 3 od lewej):", "") || "";
    const kindRaw = prompt("Typ ula: wpisz R (rodzina) lub O (odkład)", "R");
    const year = Number(prompt("Rok (np. 2025):", String(thisYear))) || thisYear;

    const id = label.trim();
    if (State.hives.some(h => h.id === id)){
      alert("Taki ul już istnieje.");
      return;
    }

    State.hives.push({
      id,
      label: id,
      place: place.trim(),
      kind: String(kindRaw||"R").toUpperCase().startsWith("O") ? "odklad" : "rodzina",
      year,
      nextVisitDate: ""
    });

    await save("hives");
    renderHives();
  }

  /* ===== Delete dispatcher ===== */
  async function handleDelete(type, id){
    if (!confirm("Usunąć?")) return;

    if (type === "fin"){
      State.finance = State.finance.filter(x => x.id !== id);
      await save("finance");
      renderFinance();
      return;
    }

    if (type === "invmove"){
      State.invMoves = State.invMoves.filter(x => x.id !== id);
      await save("invMoves");
      renderInventory();
      return;
    }

    if (type === "invitem"){
      const it = State.invItems.find(x => x.id === id);
      if (!it) return;
      if (!confirm(`Usunąć produkt "${it.name}" oraz całą historię ruchów?`)) return;

      State.invItems = State.invItems.filter(x => x.id !== id);
      State.invMoves = State.invMoves.filter(x => x.itemId !== id);

      await save("invItems");
      await save("invMoves");
      renderInventory();
      return;
    }

    if (type === "know"){
      State.knowledge = State.knowledge.filter(x => x.id !== id);
      await save("knowledge");
      renderKnowledge();
      return;
    }
  }

  /* ===== START ===== */
  document.addEventListener("DOMContentLoaded", async () => {
    try {
      await openDB();
      await loadAll();

      // NAV
      $("topNav").addEventListener("click",(e)=>{
        const btn = e.target.closest("button[data-view]");
        if (!btn) return;
        renderView(btn.dataset.view, null, true);
      });

      $("addHiveBtn").addEventListener("click", addHive);
      $("backBtn").addEventListener("click", ()=>renderView("hives", null, true));

      // Filters
      $("fltReviewBtn").addEventListener("click", ()=>{ Filters.review=!Filters.review; renderHives(); });
      $("fltQueenBtn").addEventListener("click", ()=>{ Filters.queen=!Filters.queen; renderHives(); });
      $("fltClearBtn").addEventListener("click", ()=>{ Filters.review=false; Filters.queen=false; renderHives(); });

      // Details save
      $("saveHiveMetaBtn").addEventListener("click", saveHiveMeta);

      // Reviews
      $("addReviewBtn").addEventListener("click", addReview);
      $("revClearBtn").addEventListener("click", ()=>{ $("revNote").value=""; });

      // Feedings
      $("addFeedBtn").addEventListener("click", addFeeding);
      $("feedClearBtn").addEventListener("click", ()=>{
        $("feedType").value=""; $("feedQty").value=""; $("feedNote").value="";
      });

      // Queens
      $("addQueenBtn").addEventListener("click", addQueen);
      $("qClearBtn").addEventListener("click", ()=>{
        $("qName").value=""; $("qBreederLine").value=""; $("qInsem").value="";
        $("qOpalit").value=""; $("qGentle").value=""; $("qNote").value="";
        $("qIsCurrent").checked=false;
      });

      // Finance
      $("addFinBtn").addEventListener("click", addFinance);
      $("finDate").value = todayISO();

      // Inventory
      $("invAddMoveBtn").addEventListener("click", invAddMove);
      $("invQueryBtn").addEventListener("click", invQuery);
      $("invMoveDate").value = todayISO();
      $("invQueryDate").value = todayISO();

      // Knowledge
      $("addTopicBtn").addEventListener("click", addTopic);
      $("addKnowBtn").addEventListener("click", addKnowledge);

      // Export / Import
      $("exportBtn").addEventListener("click", exportBackup);
      $("importFile").addEventListener("change", async (e)=>{
        const f = e.target.files?.[0];
        e.target.value="";
        if (!f) return;
        await importBackup(f);
      });

      // Delegation + actions
      document.body.addEventListener("click", async (e)=>{
        // deletes (global)
        const d = e.target.closest("[data-del]");
        if (d) return handleDelete(d.getAttribute("data-del"), d.getAttribute("data-id"));

        // review actions
        const ar = e.target.closest("[data-act]");
        if (!ar) return;
        const act = ar.getAttribute("data-act");
        const id  = ar.getAttribute("data-id");

        if (act === "editReview") return editReviewModal(id);
        if (act === "delReview")  return deleteReview(id);

        if (act === "editFeeding") return editFeedingModal(id);
        if (act === "delFeeding")  return deleteFeeding(id);

        if (act === "setCurrentQueen") return setCurrentQueen(id);
        if (act === "editQueen") return editQueenModal(id);
        if (act === "delQueen")  return deleteQueen(id);
      });

      // Router start
      const p = new URLSearchParams(location.search);
      const v = p.get("v") || "hives";
      const id = p.get("id");
      if (v === "details" && id) renderView("details", id, false);
      else renderView(v, null, false);

    } catch (e) {
      console.error(e);
      document.body.innerHTML = `<h1 style="color:red;padding:16px">Błąd aplikacji: ${esc(e?.message||String(e))}</h1>`;
    }
  });

})();
</script>

<script>
  // Service Worker
  if ("serviceWorker" in navigator) {
    navigator.serviceWorker.register("./sw.js").catch(console.warn);
  }
</script>

</body>
</html>
